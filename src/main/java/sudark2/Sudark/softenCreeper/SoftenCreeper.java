...
public final class SoftenCreeper extends JavaPlugin {
    ...
    public class CreeperExplode implements Listener {
        ...
        public void onExplode(EntityExplodeEvent e) {
            ...
            if (en instanceof Creeper || en instanceof EnderCrystal || en instanceof WitherSkull || en instanceof Wither) {
                // 复制方块原始状态
                List<BlockState> blocks = e.blockList().stream().map(block -> block.getState()).collect(Collectors.toList());
                ...
                new BukkitRunnable() {
                    ...
                    public void run() {
                        for (BlockState block : blocks) {
                            block.getLocation().getWorld().spawnParticle(Particle.SCULK_SOUL, block.getLocation(), 2, 0.1f, 0.1f, 0.1f, 0.2f);
